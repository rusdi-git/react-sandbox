...
const CalendarStateManager = createContext<CalendarContext>({
  state: { selected: null },
});
...
function CalHeader(props: { currentDate: Date; changeDate: (val: Date) => void }) {
  ...
  const headerText = `${format(props.currentDate, MonthFormat, {
    locale: id,
  })}-${format(props.currentDate, YearFormat, { locale: id })}`;
...
function CalRows(props: { currentDate: Date }) {
  ...
  while (day <= endDate) {
    ...
    Rows.push(
      <Box key={`day-row-${v4()}`} sx={{ display: 'flex', flexDirection: 'row' }}>
        {days}
      </Box>
    );
    days = [];
  ...
  return (
    <CalendarStateManager.Provider
      value={{
        ...
        changeSelected: setSelectedDate,
      }}
    >
